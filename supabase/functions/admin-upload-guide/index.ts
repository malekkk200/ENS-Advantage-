...
//      writer. The bucket is public so URLs never expire.
//   4. Builds simple, escaped HTML from the text with images appended
//      below it, then upserts (module_name, semester) in `guides`.
//
// NOTE: this file previously existed only as a deployed Edge Function
// (added directly, not through this repo) — it's checked in here now
// to close that drift and keep `supabase functions deploy` a reliable
