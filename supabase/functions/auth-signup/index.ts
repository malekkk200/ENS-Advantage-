...
// Frontend origins allowed to call this function via CORS.
const ALLOWED_ORIGINS = [
  'https://ens-advantage.vercel.app',
  Deno.env.get('EXTRA_ALLOWED_ORIGIN') ?? '',
  'http://localhost:3000',
  'http://127.0.0.1:3000',
  // Native app (Capacitor, mobile-app/) — without these, signup/resend
  ...
  // generic "network error" instead of ever completing. This function
  // previously echoed back a single hardcoded Access-Control-Allow-Origin
  // (the website's own domain only), which never matched a request
  // coming from the app at all — see supabase/functions/_shared/security.ts,
  // which needed this identical origin list first.
...
Deno.serve(async (req) => {
  ...
  // closure) rather than at module scope, so every response — success
  // or error — gets CORS headers matching the actual caller instead of
  // a single hardcoded origin. None of the call sites below needed to
  // change for this.
