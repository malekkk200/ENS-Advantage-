/* ═══════════════════════════════════════════════════════════════
   ...
   Device integrity: see deviceIntegrity.js — on the packaged app,
   backed by a REAL native root/jailbreak check (DeviceIntegrityPlugin,
   in mobile-app/android and mobile-app/ios) once that's built in; a
   weaker JS-only heuristic otherwise. Both read() and write() below
   refuse to run at all on a device flagged as compromised — the
   RASP-style check requested alongside this file, sitting at the one
   choke point every offline decryption (and every new offline copy
   being created) has to pass through.
═══════════════════════════════════════════════════════════════ */
