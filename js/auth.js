...
export const Auth = {
  ...
  async loadProfile() {
    ...
    if (!data) {
      ...
      // login), but a plain password login never touches either of those,
      // so if a row is ever missing for any other reason, repair it here
      // too. RLS forbids the client from writing user_profiles itself, so
      // this is proxied through a service-role function that can only
      // ever touch the caller's own row. See ensure-profile/index.ts.
      // Wrapped so a network hiccup here just falls back to today's
      // existing (silent) behavior instead of throwing into callers —
      // some of which (onAuthStateChange's SIGNED_IN handler) still need
      // their own render() to run right after this either way.
      try {
